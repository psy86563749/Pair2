...
double randf()
{
    double grade;
    double n;   //保存随机数
  	double down=1.5,top=4;//区间范围
    n=(double)rand()/RAND_MAX;
    //产生0-1之间的随机数，RAND_MAX是生成随机数的最大值
    grade=down+n*(top-down);
    return grade;
}
inline void DataBaseRoll(string& name, string& sex, string& major, string& attendance ,double &jd,double &efficiency) 
{//随机生成函数
    ...
    if(flag==1)
    {
        ...
        {
             int b=rand()%100;
             if(b<80)  {attendance[i]='0';}
             else {attendance[i]='1';count++;}
        }
    }
    else 
    {
        ...
        {
             int b=rand()%100;
             if(b>96)  {attendance[i]='0';}
             else {attendance[i]='1';count++;}
        }

    }
    efficiency=count*1.0/20;
